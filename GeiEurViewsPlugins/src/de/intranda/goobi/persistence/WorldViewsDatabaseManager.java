...
public class WorldViewsDatabaseManager {
		...
		try {
			...
			if (data.getResourceID() == null) {
				...
				Object[] parameter = { data.getProzesseID(), data.getDocumentType(), data.getMaintitleOriginal(),
						...
						data.getNumberOfPages(), data.getShelfmark(), data.getMaintitleGerman(),
						data.getMaintitleEnglish(),
						...
						authors.toString(), publishers.toString()
				...
				Object[] parameter = { data.getProzesseID(), data.getDocumentType(), data.getMaintitleOriginal(),
						...
						data.getEducationLevel(), data.getEdition(), data.getIsbn(), data.getPhysicalLocation(),
						authors.toString(), publishers.toString(), data.getResourceID()
	 ...
	 * @throws SQLException
	 */
	private static void deleteMetadata(Integer resourceID, Integer prozesseID, String type, Connection connection,
	 ...
	 * @throws SQLException
	 */
	private static void deleteStrings(Integer resourceID, Integer processeID, String type, Connection connection,
			QueryRunner run) throws SQLException {
	...
	private static void insertMetadata(QueryRunner run, Connection connection, Integer resourceID, Integer prozesseID,
			String type, ComplexMetadataObject obj) {
		StringBuilder sql = new StringBuilder();
		...
		if (type.equals("publisher") || type.equals("corporation") || type.equals("volumeCorporation")) {
			...
			Object[] parameter = { resourceID, prozesseID, type, pub.getRole(), pub.getNormdataAuthority(),
					pub.getNormdataValue(), pub.getName() };
			try {
			...
			sql.append(COLUMN_RESOURCE_PROCESSID);
			sql.append(", type, role, normdataAuthority, normdataValue , firstValue) VALUES (?, ?, ?, ?, ?, ?, ?);");
			Object[] parameter = { resourceID, prozesseID, type, loc.getRole(), loc.getNormdataAuthority(),
					loc.getNormdataValue(), loc.getName() };
			try {
				run.insert(connection, sql.toString(), dummyHandler, parameter);
			} catch (SQLException e) {
				logger.error(e);
			...
			sql.append(COLUMN_RESOURCE_PROCESSID);
			sql.append(
					", type, role, normdataAuthority, normdataValue , firstValue, secondValue) VALUES (?, ?, ?, ?, ?, ?, ?, ?);");
			Object[] parameter = { resourceID, prozesseID, type, aut.getRole(), aut.getNormdataAuthority(),
					aut.getNormdataValue(), aut.getFirstName(), aut.getLastName() };
			try {
				run.insert(connection, sql.toString(), dummyHandler, parameter);
			} catch (SQLException e) {
				logger.error(e);
	...
	private static void insertListItem(QueryRunner run, Connection connection, int resourceId, int processId,
	...
	public static ResouceMetadata getResourceMetadataOld(Integer processId) throws SQLException {
		...
		try {
			...
			ResouceMetadata ret = new QueryRunner().query(connection, sql.toString(),
					WorldViewsDatabaseManager.resultSetToResourceMetadataHandler);
			return ret;
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
			...
			for (Image curr : currentImages) {
				...
				if (curr.getImageId() == null) {
					...
					Object[] parameter = { curr.getProcessId(), curr.getFileName(), curr.getOrder(),
							...
							curr.isDisplayImage(), StringUtils.isEmpty(curr.getLicence()) ? null : curr.getLicence(),
							curr.isRepresentative(), curr.getCopyright(), curr.getPlaceholder() };
					if (logger.isDebugEnabled()) {
						logger.debug(sql.toString() + ", " + Arrays.toString(parameter));
					}
					...
					Object[] parameter = { curr.getProcessId(), curr.getFileName(), curr.getOrder(),
							StringUtils.isEmpty(curr.getStructType()) ? null : curr.getStructType(),
							curr.isDisplayImage(), StringUtils.isEmpty(curr.getLicence()) ? null : curr.getLicence(),
							curr.isRepresentative(), curr.getCopyright(), curr.getPlaceholder(), curr.getImageId() };
					if (logger.isDebugEnabled()) {
						logger.debug(sql.toString() + ", " + Arrays.toString(parameter));
					}
					run.update(connection, sql.toString(), parameter);
	...
	public static List<Image> getImages(int processId) throws SQLException {
		...
		try {
			...
			List<Image> ret = new QueryRunner().query(connection, sql.toString(),
					WorldViewsDatabaseManager.resultSetToImageListHandler);
			return ret;
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
	...
	public static List<Context> getDescriptionList(Integer processId) throws SQLException {
		...
		try {
			...
			List<Context> ret = new QueryRunner().query(connection, sql.toString(),
					WorldViewsDatabaseManager.resultSetToDescriptionListHandler);
			return ret;
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
	...
	private static void getLists(BibliographicMetadata data) throws SQLException {
		...
		try {
			...
			List<String> languages = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToStringListHandler, lparameter);
			...
			String languageMainTitle = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToStringHandler, languageMainTitleParameter);
			if (StringUtils.isNotBlank(languageMainTitle)) {
				data.setLanguageMainTitle(languageMainTitle);
			}

			Object[] languageVolumeTitleParameter = { data.getResourceID(), data.getProzesseID(),
					"languageVolumeTitle" };
			String languageVolumeTitle = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToStringHandler, languageVolumeTitleParameter);
			...
			List<Person> book = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPersonListHandler, bookAuthor);
			data.setPersonList(book);

			List<Person> vol = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPersonListHandler, volumeAuthor);
			data.setVolumePersonList(vol);

			List<Corporation> corp = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPublisherListHandler, corporation);
			data.setCorporationList(corp);

			List<Corporation> pub = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPublisherListHandler, publisher);
			data.setPublisherList(pub);

			List<Corporation> vCorp = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPublisherListHandler, volumeCorporation);
			data.setVolumeCorporationList(vCorp);

			List<Location> countryList = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToLocationListHandler, country);
			data.setCountryList(countryList);

			List<Location> loc = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToLocationListHandler, location);
	...
	private static ResultSetHandler<List<BibliographicMetadata>> resultSetToBibliographicDataListHandler = new ResultSetHandler<List<BibliographicMetadata>>() {
		@Override
			...
			try {
				while (rs.next()) {
					...
					if (desc.getLanguage() != null) {
						...
						logger.error(
								"Unable to load description " + desc.getDescriptionID() + ". No language provided");
					}
				}
			} finally {
				if (rs != null) {
			...
			for (Transcription current : transcriptionList) {
				...
				if (current.getTranscriptionID() == null) {
					...
					Object[] parameter = { current.getProzesseID(),
							...
							StringUtils.isEmpty(current.getAvailability()) ? null : current.getAvailability(),
							StringUtils.isEmpty(current.getLicence()) ? null : current.getLicence(),
							current.isOriginalLanguage(), current.getTranscriptionID() };
					if (logger.isDebugEnabled()) {
						logger.debug(sql.toString() + ", " + Arrays.toString(parameter));
					}
					run.update(connection, sql.toString(), parameter);
	...
	public static List<BibliographicMetadata> getBibliographicData(String query) throws SQLException {
		...
		if (!StringUtils.isEmpty(query)) {
			sql += QUERY_WHERE + COLUMN_RESOURCE_MAINTITLE_ORIGINAL + " LIKE '%" + StringEscapeUtils.escapeSql(query)
					+ "%'" + " OR " + COLUMN_RESOURCE_MAINTITLE_ENGLISH + " LIKE '%"
					+ StringEscapeUtils.escapeSql(query) + "%'" + " OR " + COLUMN_RESOURCE_MAINTITLE_ORIGINAL
					+ " LIKE '%" + StringEscapeUtils.escapeSql(query) + "%'" + " OR " + COLUMN_RESOURCE_RESOURCEID
					+ " LIKE '%" + StringEscapeUtils.escapeSql(query) + "%';";
		}
		Connection connection = null;
		try {
			connection = MySQLHelper.getInstance().getConnection();
	...
	public static List<Map<String, String>> getResource(String query) throws SQLException {
		...
		try {
			...
			// get IDs
			List<Integer> idList = new QueryRunner().query(connection, sql.toString(),
					MySQLHelper.resultSetToIntegerListHandler);
		...
		try {
			...
			if (contribution.getContributionId() == null) {
				...
				Object[] parameter = { contribution.getProcessId(),
						...
						StringUtils.isEmpty(contribution.getContent()) ? null : contribution.getContent(),
						StringUtils.isEmpty(contribution.getContext()) ? null : contribution.getContext(),
						contribution.isOriginalLanguage(), contribution.getContributionId() };
			...
			for (SimpleMetadataObject lang : translatorListOriginal) {
				insertListItem(run, connection, contribution.getContributionId(), contribution.getProcessId(),
						"translator", lang.getValue());
			}
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
	...
	public static List<Contribution> getContributions(int processId) throws SQLException {
		...
		String sql = QUERY_SELECT_FROM + TABLE_CONTRIBUTION + QUERY_WHERE + COLUMN_CONTRIBUTION_PROCESSID + " = "
				+ processId;
		Connection connection = null;
		try {
			connection = MySQLHelper.getInstance().getConnection();
			...
			for (Contribution contribution : contributionList) {
				...
				List<String> translators = new QueryRunner().query(connection, sql,
						WorldViewsDatabaseManager.resultSetToStringListHandler, lparameter);
				for (String s : translators) {
		...
		try {
			...
			if (list != null) {
				for (Source current : list) {
					...
					Object[] parameter = { processId,
							current.getData() == null ? null : current.getData().getProcessId(),
							current.isMainSource() };
	...
	public static List<Source> getSourceList(int processId) throws SQLException {
		...
		try {
			...
			List<Source> ret = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToSourceListHandler);
	...
	public static List<StringPair> getKeywordList(Integer processId) throws SQLException {
		...
		try {
			...
			List<StringPair> ret = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToTopicListHandler);
			return ret;
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
	...
	public static void saveContribtutionDescription(ResourceAnnotationPlugin plugin) throws SQLException {
		...
		try {
			...
			if (plugin.getId() == null) {
				...
				Object[] parameter = { plugin.getProcessId(),
	...
	public static void getContributionDescription(ResourceAnnotationPlugin plugin) throws SQLException {
		...
		try {
			...
			Map<String, String> ret = new QueryRunner().query(connection, sql.toString(),
					WorldViewsDatabaseManager.resultSetToMapHandler);
			if (!ret.isEmpty()) {
			...
			String metadata = "SELECT * FROM " + TABLE_METADATA
					+ " WHERE resourceID = ? AND prozesseID = ? AND type = ?";
			Object[] parameter = { plugin.getId(), plugin.getProcessId(), "contribution" };
			List<Person> per = new QueryRunner().query(connection, metadata,
					WorldViewsDatabaseManager.resultSetToPersonListHandler, parameter);
			...
			List<String> dcList = new QueryRunner().query(connection, strings,
					WorldViewsDatabaseManager.resultSetToStringListHandler, parameterDC);
	...
	public static List<Language> getLanguageList(String searchTerm) throws SQLException {
		...
		try {
			connection = MySQLHelper.getInstance().getConnection();
			List<Language> ret = new QueryRunner().query(connection, sql.toString(),
					WorldViewsDatabaseManager.resultSetToLanguageList);
			return ret;
		} finally {
			if (connection != null) {
				MySQLHelper.closeConnection(connection);
	...
	public static ResouceMetadata getResourceMetadata(Integer id) throws SQLException {
		...
		try {
			...
			ResouceMetadata metadata = new QueryRunner().query(connection, sql,
					WorldViewsDatabaseManager.resultSetToResouceMetadataHandler);
			if (metadata != null) {
				...
				List<Person> res = new QueryRunner().query(connection, sql,
						WorldViewsDatabaseManager.resultSetToPersonListHandler, resourceAuthor);
				...
				List<String> dcs = new QueryRunner().query(connection, sql,
						WorldViewsDatabaseManager.resultSetToStringListHandler, resourceDC);
				if (dcs != null) {
	...
	public static void saveResouceMetadata(ResouceMetadata data) throws SQLException {
		...
		try {
			...
			if (data.getId() == null) {
				...
				Integer id = run.insert(connection, sql.toString(), MySQLHelper.resultSetToIntegerHandler,
						data.getProcessId(), data.getBibliographicDataId(), data.getResourceType(),
						data.getResourceTitleOriginal(), data.getResourceTitleGerman(), data.getResourceTitleEnglish(),
						data.getStartPage(), data.getEndPage(), data.getSupplier());
				if (id != null) {
					data.setId(id);
				}
				...
				run.update(connection, sql.toString(), data.getProcessId(), data.getBibliographicDataId(),
						data.getResourceType(), data.getResourceTitleOriginal(), data.getResourceTitleGerman(),
						data.getResourceTitleEnglish(), data.getStartPage(), data.getEndPage(), data.getSupplier(),
						data.getId());
			}
