...
public @Data class ResourceDescriptionPlugin implements IStepPlugin, IPlugin {
	...
	private void initializeResourceTypes() {
		...
		possibleTypes
				.add(new SelectItem("Abbildungen - Sammelbild", "TODO Auto-generated method stub   -   Sammelbild"));
	...
	public void setImage(Image image) {
		...
		HttpSession session = (HttpSession) context.getExternalContext().getSession(false);
		String currentImageURL = ConfigurationHelper.getTempImagesPathAsCompleteDirectory() + session.getId() + "_"
				+ image.getFileName() + "_large_" + ".png";
		try {
	...
	public String getBild() {
		if (image == null) {
			...
			HttpSession session = (HttpSession) context.getExternalContext().getSession(false);
			String currentImageURL = baseUrl + ConfigurationHelper.getTempImagesPath() + session.getId() + "_"
					+ image.getFileName() + "_large_" + ".png";
	...
	public String getBildIIIFUrl() throws IOException, InterruptedException, SwapException, DAOException {
		if (image != null) {
			StringBuilder sb = new StringBuilder(
					ConfigPlugins.getPluginConfig(this).getString("presentation.url", "http://localhost:8080/viewer"));
	 ...
	 * 
	 * 
	 * @return a list of language Strings of transcriptions as well as the
	 *         string "image" to represent the image display
	 */
	...
	private void setDefaultValues(Transcription transcription) {
		...
		transcription.setAvailability(ConfigPlugins.getPluginConfig(this).getString(keyAvailability,
				TeiExportPlugin.DEFAULT_TEXT_AVAILABILITY));
	}
	 ...
	 * 
	 * @param language
	 * @return the first context/description with the given language. Or null if
	 ...
	 * 
	 * @param language
	 * @return the first transcription with the given language. Or null if no
	 *         such transcription exists
	 */
